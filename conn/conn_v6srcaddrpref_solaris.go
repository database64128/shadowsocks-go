...
func setIPv6SourceAddressPreference(fd int, network string, pref IPv6SourceAddressPreference) error {
	...
	if err := unix.SetsockoptInt(fd, unix.IPPROTO_IPV6, unix.IPV6_SRC_PREFERENCES, value); err != nil {
		return fmt.Errorf("failed to set socket option IPV6_SRC_PREFERENCES to %#x: %w", value, err)
	}
	return nil
}
